...
(
	...
	output wire [15:0] input_mux_output,
	input wire uart_rx,
	output wire uart_tx
);
	...
	wire [31:0] instr_mem_output;
	wire [31:0] Extend26_32;
	wire [31:0] Extend16_32;
	wire [31:0] PAM_output;
	...
	wire [31:0] address_out_proc;
	wire [31:0] hd_output;
	wire [31:0] selected_instruction;
	wire [5:0] exec_proc, intrpt_val;
	wire [4:0] ALUcode, PRBM_out;
	wire [15:0] uart_receive;
	wire [15:0] pre_input_mux_output;
	...
	wire [3:0] mil, cent, dez, uni, cent_pc, dez_pc, uni_pc;
	wire [3:0] cent_reg_address, dez_reg_address, uni_reg_address;
	...
	Memoria_Instrucoes IM (
		.proc_num(decoded_exec_proc),
		.escrever_SO(load_os),
		.flag_escrita(write_instr_mem),
		.endereco_escrita(data_3[15:0]), 
		.endereco_leitura(address_out),
		.dado_entrada(hd_output),
		.clk_escrita(clk_div),
		.clk_leitura(clk_auto), 
		.saida_inst_mem(instr_mem_output)
	);
	...
	/*
	 * Seleção da origem da entrada:
	 *
	 * interface_mode = 0 -> usa switches físicos da FPGA
	 * interface_mode = 1 -> usa dados recebidos pela UART
	 */
	Mux_16 PRE_INPUT_MUX (
		.input_data_1(switches),
		.input_data_2(uart_receive),
		.mux_output(pre_input_mux_output),
		.mux_select(receive | interface_mode)
	);
	...
	Multiplexador_PC BM (
		...
		.pc_out(address_out),
		.saida_big_mult(address_in)
	);
	/* NETWORK COMMUNICATION MODULE */
	wire send_to_interface;

	assign send_to_interface =
		send |
		(selected_instruction[31:26] == 6'b011001);
	
	UART_Manager UART_M (
		.send(send_to_interface),
		.lcd_send(trd_msg),
		.clk_auto(clk_auto),

		.output_data(output_value[15:0]),
		.lcd_code(selected_instruction[7:0]),
